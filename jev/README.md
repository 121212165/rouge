...
用户三个问题（手机版怎么提交数据 / 怎么释放技能 / 视图不好看）。先在 390x844 真机视口量了一遍，
结论比反馈本身更难看：

| 实测 | 值 | 含义 |
|---|---|---|
| `feedback` 可点入口 | **false** | 反馈弹窗只绑在键盘 F 键上 → **手机玩家根本提交不了数据** |
| 遥测端点 | **null** | 只有带 `?tlUrl=` 才上传；Pages 上没人带 → 事件全埋在 localStorage |
| 格子 | **14px** | 25x15 全图挤进 367px |
| 图例高度 | **214px** | 和地图（220px）一样高，吃掉一整屏 |
| 地图占页面 | **13%** | 文档高 1683 vs 视口 844，两屏滚动 |

此前 4 个真 bug 全部来自这份诊断数据 —— 手机上交不回来，等于把唯一的反馈渠道关掉了。

第六轮（`--round=6` → `mobile-view-plan.json`）判定：

| 问题 | 结论 | 置信 |
|---|---|---|
| board_fit | **legend_folds 0.94** | 0.92 —— 迄今最果断的一问 |
| legend_on_mobile | **fold_button 0.72** | 0.63 |
| skill_discovery | **label_hint 0.69** | 0.58 |
| feedback_unreachable | 手机玩家完全无法提交反馈 | **0.92** |
| data_channel | tap_entry 0.49 vs auto_endpoint 0.46 | 0.31 —— **没收敛** |
| map_dominates / one_screen | 0.58 / **0.51** | 它不相信光折叠图例就够 |

落地与对判定的偏离：
- 图例默认折叠（窄屏），点标题行展开，选择记进 `localStorage`。
- 反馈加可点入口：底部动作条一个「反馈」按钮 + 控制行一个内联按钮。
  `data_channel` 两选项没收敛，我按"没入口时端点也没用"取 tap_entry；
  **没有**内置默认遥测端点 —— 那要在公开站点里硬编码一个上传地址，
  归谁运营、存多久、算不算收集他人数据，是用户的决定，不该由我替他做。
- 技能按钮写清「灵气25」并在首次释放前描边脉冲；实测动作条为
  `破甲 灵气25 / 连击 灵气30 / 待一回合 / 反馈`。
- `one_screen` 只有 0.51 是对的：光折叠图例状态带仍 171px 把地图顶到 248px。
  所以额外把状态压成细带（境界/经验/防御/击杀 在窄屏隐藏）、地图上方只留
  气血条 + 关键数字 + 命格/一气/煞气，并加一条固定在动作条上方的**日志浮条**
  （日志面板在折叠线以下，手机上战斗反馈本来看不见）。
- 实测回收：文档高 1683 → **1153**，图例 214 → **48**，状态带 171 → **134**，
  地图占视口 13% → **26%**，反馈入口 **true**。顺手修掉「命格 命格 金」这类重复标签。

这些数字现在钉在 `check_touch.py` 里当回归断言（图例 >90px、状态带 >150px、
地图底被动作条压住、无反馈入口、无日志浮条、重复标签 → 全部判失败）。

**格子 14px 这一条没解决，也不该假装解决了**：竖屏 390px 宽放 25 列就是这个数，
罗盘让"点不准"不再是问题，但"看不清"仍在。真正的杠杆是横屏（844px 宽 → 约 33px），
Jev 在 `board_fit` 里给 landscape 的分很低（没进前三），所以这轮没做。

## 外部素材：接进来之前先量了它能不能用

用户用豆包按 `ART-PROMPT.md` 生成了一批 128x128 素材。规格全对（RGBA、tile-floors 是 384x128 三联），
`style-palette.png` 的像素质量也确实好。但**规格对不等于能用**：

1. **背景不是提示词要求的 `#080d1c`** —— 每张角落色从 (12,30,54) 到 (35,52,59) 不等，
   只有 3/26 命中。直接铺会在每格上露出一圈比地面更亮的方块，25x15 网格下变成新的墙噪声。
   → `harness/prep_assets.py` 从四条边**洪水填充**抠背景（不是全局色差阈值，
   否则角色自身的暗部描边会被打穿成洞），原图留在 `assets/raw/`。
2. **缩到真实格子尺寸就糊了** —— 合成模拟图后肉眼看：26px 勉强可辨，
   **14px（手机实际格宽）下僵尸/煞尸/盾卫全是一团色块**。
   直接替换字形会让游戏变差，而且违背 Jev 选 pixel_dao 的理由（格子边界天然清晰）。

所以素材**不进地图格子**，只用在四个不与可读性抢位置的地方：

| 位置 | 尺寸 | 为什么这里能用 |
|---|---|---|
| 一气连环引爆特效 | 约 206px 覆盖在目标格 | 一闪而过的大图，正好补上 `verb_final_clear 0.18` 缺的那张脸 |
| 图例敌人立绘 | 30px，与字形并排 | 立绘管"这是什么东西"，字形管"地图上那格长这样"，两者互补不是互斥 |
| 法宝图标 | 22px | 法宝本来就只有文字，图标是净增信息 |
| 标题印章 | 30px | 不参与战斗判读 |

每个 `<img>` 都带 `onerror="this.remove()"`：素材缺失或加载失败就退回原来的字形，
**不会变成一堆破图**。闸门新增 `assets_present` 否决项 —— 代码引用的每张图必须真在盘上
（法宝图标按 relic id 拼路径，只从 `RELICS` 数组取 id；早先版本连 `SHA` 的 id 也扫进去，
凭空报出 8 个"缺图"，这种假失败比没检查更糟）。

首屏实测：按需只加载 9 张 / 231KB，加 index.html 共 **325KB**。

顺带又抓到一处自己写的测试夹具 bug：`check_touch.py` 的 `CLEAR` 要求"整行整列不许有掉落物"，
